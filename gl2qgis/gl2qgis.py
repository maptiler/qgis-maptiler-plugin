...
def parse_symbol_layer_as_renderer(json_layer: dict, context: QgsMapBoxGlStyleConversionContext):
    json_layout = json_layer.get("layout")
    if not json_layout:
        context.pushWarning(f"{context.layerId()}: Style layer has no layout property, skipping.")
        return None, False

    json_symbol_placement = json_layout.get("symbol-placement")
    json_text_field = json_layout.get("text-field")
    if json_symbol_placement == "line" and not json_text_field:
        dd_properties = QgsPropertyCollection()
        spacing = 0
        json_spacing = json_layout.get("symbol-spacing")
        if json_spacing:
            if isinstance(json_spacing, (int, float)):
                spacing= json_spacing * context.pixelSizeConversionFactor()
            elif isinstance(json_spacing, dict):
                dd_properties.setProperty(QgsSymbolLayer.PropertyInterval,
                                          parse_interpolate_by_zoom(json_spacing, context,
                                                                    context.pixelSizeConversionFactor()))
            elif isinstance(json_spacing, list):
                dd_properties.setProperty(QgsSymbolLayer.PropertyInterval,
                                          parse_value_list(json_spacing, PropertyType.Numeric, context,
                                                           context.pixelSizeConversionFactor(), 255))
            else:
                context.pushWarning(f"{context.layerId()}: Skipping unsupported symbol-spacing type ({type(json_spacing).__name__})" )
        else:
            # Defaults to 250
            spacing = 250 * context.pixelSizeConversionFactor()

        rotate_markers = True
        alignment = json_layout.get("icon-rotation-alignment")
        if alignment:
            if alignment == "map" or alignment == "auto":
                rotate_markers = True
            elif alignment == "viewport":
                rotate_markers = False

        marker_dd_properties = QgsPropertyCollection()
        rotation = 0.0
        json_icon_rotate = json_layout.get("icon-rotate")
        if json_icon_rotate:
            if isinstance(json_icon_rotate, (int, float)):
                rotation = float(json_icon_rotate)
            elif isinstance(json_icon_rotate, dict):
                marker_dd_properties.setProperty(QgsSymbolLayer.PropertyAngle,
                                                 parse_interpolate_by_zoom(json_icon_rotate, context, context.pixelSizeConversionFactor()))
            elif isinstance(json_icon_rotate, list):
                marker_dd_properties.setProperty(QgsSymbolLayer.PropertyAngle,
                                                 parse_value_list(json_icon_rotate, PropertyType.Numeric, context, context.pixelSizeConversionFactor()))
            else:
                context.pushWarning(f"{context.layerId()}: Skipping unsupported icon-rotate type ({type(json_icon_rotate).__name__})")

        line_symbol = QgsMarkerLineSymbolLayer(rotate_markers, spacing if spacing > 0 else 1)
        line_symbol.setOutputUnit(context.targetUnit())
        line_symbol.setDataDefinedProperties(dd_properties)

        if spacing < 1:
            line_symbol.setPlacement(QgsTemplatedLineSymbolLayerBase.CentralPoint)
        ...
        sprite_property = ""
        sprite_size_property = ""
        sprite = core_converter.retrieveSpriteAsBase64(json_layout.get("icon-image"), context, sprite_size,
                                                       sprite_property, sprite_size_property)
        if sprite:
            marker_layer.setPath(sprite)
            marker_layer.setSize(sprite_size.width())
            marker_layer.setSizeUnit(context.targetUnit())

            if sprite_property:
                marker_dd_properties.setProperty(QgsSymbolLayer.PropertyName,
                                                 QgsProperty.fromExpression(sprite_property))
                marker_dd_properties.setProperty(QgsSymbolLayer.PropertyWidth,
                                                 QgsProperty.fromExpression(sprite_size_property))

        json_icon_size = json_layout.get("icon-size")
        if json_icon_size:
            size = 1.0
            property = QgsProperty()
            if isinstance(json_icon_size, (int, float)):
                size = float(json_icon_size)
                if sprite_size_property:
                    marker_dd_properties.setProperty(QgsSymbolLayer.PropertyWidth,
                                                     QgsProperty.fromExpression(f"with_variable('marker_size',{sprite_size_property},{size}*@marker_size)"))
            elif isinstance(json_icon_size, dict):
                property = parse_interpolate_by_zoom(json_icon_size, context, 1)
            elif isinstance(json_icon_size, list):
                context.pushWarning(f"{context.layerId()}: Skipping non-implemented icon-size type ({type(json_icon_size).__name__})")

            marker_layer.setSize(size * sprite_size.width())
            if property.expressionString():
                if sprite_size_property:
                    marker_dd_properties.setProperty(QgsSymbolLayer.PropertyWidth,
                                                     QgsProperty.fromExpression(f"with_variable('marker_size',{sprite_size_property},({property.expressionString})*@marker_size)"))
                else:
                    marker_dd_properties.setProperty(QgsSymbolLayer.PropertyWidth,
                                                     QgsProperty.fromExpression(f"({sprite_size.width()})*{property.expressionString()}"))

        marker_layer.setDataDefinedProperties(marker_dd_properties)
        marker_layer.setAngle(rotation)
        line_symbol.setSubSymbol(marker_layer)

        symbol = QgsLineSymbol(line_symbol)

        # Set render units
        symbol.setOutputUnit(context.targetUnit())
        line_symbol.setOutputUnit(context.targetUnit())

        renderer_style = QgsVectorTileBasicRenderer()
        renderer_style.setGeometryType(QgsWkbTypes.LineGeometry)
        renderer_style.setSymbol(symbol)

        return renderer_style, True
    elif "icon-image" in json_layout:
        json_paint = json_layer.get("paint")
        core_converter = QgsMapBoxGlStyleConverter()
        sprite_size = QSize()
        sprite_property = ""
        ...
        sprite = core_converter.retrieveSpriteAsBase64(json_layout.get("icon-image"), context, sprite_size,
                                                       sprite_property, sprite_size_property)
        if sprite:
            raster_marker = QgsRasterMarkerSymbolLayer()
            raster_marker.setPath(sprite)
            raster_marker.setSize(sprite_size.width())
            raster_marker.setSizeUnit(context.targetUnit())
            ...
            if json_icon_size:
                ...
                if isinstance(json_icon_size, (int, float)):
                    ...
                    context.pushWarning(f"{context.layerId()}: Skipping non-implemented icon-size ({type(json_icon_size).__name__}).")

                raster_marker.setSize(size * sprite_size.width())
                if property.expressionString():
                    if sprite_size_property:
                        marker_dd_properties.setProperty(QgsSymbolLayer.PropertyWidth,
                                                         QgsProperty.fromExpression(f"with_variable('marker_size',{sprite_size_property},({property.expressionString()})*@marker_size)"))
                        ...
                        marker_dd_properties.setProperty(QgsSymbolLayer.PropertyWidth,
                                                         QgsProperty.fromExpression(f"({property.expressionString()})*{sprite_size.width()}"))
            ...
            if json_icon_rotate:
                if isinstance(json_icon_rotate, (int, float)):
                    ...
                    marker_dd_properties.setProperty(QgsSymbolLayer.PropertyAngle,
                                                     parse_value_list(json_icon_rotate, PropertyType.Numeric, context, context.pixelSizeConversionFactor(), 255))
                else:
                    context.pushWarning(f"{context.layerId()}: Skipping unsupported icon-rotate type ({type(json_icon_rotate).__name__}).")

            icon_opacity = -1.0
            json_icon_opacity = json_paint.get("icon-opacity")
            if json_icon_opacity:
                if isinstance(json_icon_opacity, (int, float)):
                    icon_opacity = float(json_icon_opacity)
                elif isinstance(json_icon_opacity, dict):
                    marker_dd_properties.setProperty(QgsSymbolLayer.PropertyOpacity,
                                                     parse_interpolate_by_zoom(json_icon_opacity, context, 100))
                elif isinstance(json_icon_opacity, list):
                    marker_dd_properties.setProperty(QgsSymbolLayer.PropertyOpacity,
                                                     parse_value_list(json_icon_opacity, PropertyType.Numeric, context,
                                                                      100, 255))
                else:
                    context.pushWarning(f"{context.layerId()}: Skipping unsupported icon-opacity type ({type(json_icon_opacity).__name__})")

            raster_marker.setDataDefinedProperties(marker_dd_properties)
            raster_marker.setAngle(rotation)
            if icon_opacity >= 0:
                raster_marker.setOpacity(icon_opacity)


            # TO-DO pokracovat tady, problem s instantializaci
            marker_symbol = QgsMarkerSymbolLayer([raster_marker])
            # marker_symbol = QgsSymbol.defaultSymbol(QgsWkbTypes.LineGeometry)
            marker_symbol.setLayer(raster_marker)
            renderer_style = QgsVectorTileBasicRenderer()
            renderer_style.setGeometryType(QgsWkbTypes.PointGeometry)
            renderer_style.setSymbol(marker_symbol)
